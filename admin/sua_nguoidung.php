...
<?php
		$username=$_GET['idnd'];
		$rows=mysqli_query($link,"SELECT * from nguoidung where username='$username'");
		$row=mysqli_fetch_assoc($rows);
?>
<form action="update_nguoidung.php?idnd=<?php echo $username;?>" method="post" name="frm" onsubmit="return kiemtra()" enctype="multipart/form-data">
	<table>
			<tr class="tieude_themsp">
				<td colspan=2>Sửa Người Dùng </td>
			</tr>
			<tr>
				<td>Username</td><td><input type="text" disabled="disabled" name="user"  value="<?php echo $row['username'] ?>"/></td>
			</tr>
    		<tr>
            	<td>Tên ND</td><td><input type="text" name="tennd" value="<?php echo $row['tennguoidung'] ?>"/></td>
            </tr>
			<tr>
            	<td>Email</td><td><input type="text" name="email"  value="<?php echo $row['email'] ?>"/></td>
            </tr>
			<tr>
            	<td>Điện thoại</td><td><input type="text" name="dienthoai"  value="<?php echo $row['sodienthoai'] ?>"/></td>
            </tr>
            <tr>
